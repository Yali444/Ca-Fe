/**
 * Geocode Roasters Modi'in branch
 */
...
async function geocodeGoogle(place: Place, apiKey: string): Promise<GeocodeResult | null> {
  try {
    const queries = [
      `Roasters, ${place.address}, ${place.city}, Israel`,
      `${place.address}, ${place.city}, Israel`,
      `${place.name}, ${place.city}, Israel`,
      `W234+HR ${place.city}, Israel`,
    ];
...
async function main() {
  ...
  console.log('🔍 Geocoding Roasters Modi\'in...\n');
  ...
  if (geocodeResult) {
    ...
    const outputPath = path.join(process.cwd(), 'scripts', 'roasters-modiin-geocoding-result.json');
