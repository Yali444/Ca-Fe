...
async function geocodeGoogle(address: string, city: string, apiKey: string): Promise<GeocodeResult | null> {
  try {
    ...
    const queries = [
      ...
      `Horowitz 36 ${englishCity} Israel`,
      `${address}, ${city}, Israel`
    ];
...
async function main() {
  ...
  if (result) {
    ...
    const resultsPath = path.join(process.cwd(), 'geocoding-results-velev.json');
