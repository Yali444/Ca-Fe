/**
 * Verify and update all coordinates in roasteries.ts using Google Geocoding API
 */
...
async function main() {
  ...
  for (let i = 0; i < CAFES.length; i++) {
    const cafe = CAFES[i];
    console.log(`[${i + 1}/${CAFES.length}] ${cafe.name} - ${cafe.address}, ${cafe.city}`);
    
    const geocodeResult = await geocodeGoogle(cafe.name, cafe.address, cafe.city, apiKey);
    ...
    if (geocodeResult) {
      ...
      if (needsUpdate) {
        ...
        console.log(`     Current: ${cafe.coordinates.lat}, ${cafe.coordinates.lng}`);
  ...
  const outputPath = path.join(process.cwd(), 'scripts', 'coordinate-verification-results.json');
  ...
  if (needsUpdate > 0) {
    console.log(`\n📋 Cafes that need coordinate updates:\n`);
    results
